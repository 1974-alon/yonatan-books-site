const translations = {
  ...
  function openPartner(index) {
    const partner = partnersData[appState.lang][index];
    const labels = translations[appState.lang].partners;
  ...
  function attachRipple(target, event) {
    ...
    circle.className = 'ui-button__ripple';
    circle.style.width = circle.style.height = `${diameter}px`;
    circle.style.left = `${event.clientX - rect.left - diameter / 2}px`;
    circle.style.top = `${event.clientY - rect.top - diameter / 2}px`;
    const oldRipple = target.querySelector('.ui-button__ripple');
